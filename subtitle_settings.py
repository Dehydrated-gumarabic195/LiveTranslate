...
def _make_image_row(current_path: str, on_change):
    """Create a background image selector row. Returns (layout, line_edit, color_controls_to_disable)."""
    row = QHBoxLayout()
    row.setSpacing(6)

    row.addWidget(QLabel(t("subwin_bg_image")))
...
class _LineConfigWidget(QWidget):
    ...
    def _build_ui(self):
        ...
        # Header row: ▲▼ + enabled + type combo + remove button
        hdr = QHBoxLayout()
        hdr.setSpacing(6)
        ...
        # Language selector (visible only for translation type)
        ...
        if self._removable:
            ...
            rm_btn.setStyleSheet(
                "QPushButton { color: #c55; border: 1px solid #c55; border-radius: 3px; padding: 0 8px; }"
                "QPushButton:hover { background: rgba(200,80,80,40); }"
            )
        ...
        row1.addWidget(QLabel(t("subwin_font")))
        ...
        row1.addWidget(QLabel(t("subwin_font_size")))
        ...
        lbl_opacity = QLabel(t("subwin_opacity"))
        row2.addWidget(lbl_opacity)
        self._opacity_slider = QSlider(Qt.Orientation.Horizontal)
        self._opacity_slider.setRange(0, 255)
        self._opacity_slider.setValue(self._cfg.get("opacity", 255))
        ...
        row2.addWidget(QLabel(t("subwin_align")))
        ...
        row2.addWidget(self._align_combo)
        ...
        row3.addWidget(self._outline_check)

        row3.addWidget(QLabel(t("subwin_outline_color")))
        ...
        row3.addWidget(self._outline_color_btn)

        row3.addWidget(QLabel(t("subwin_outline_width")))
        ...
        row3.addWidget(self._outline_width)
        ...
        anim_row.addWidget(QLabel(t("subwin_scroll_speed")))
        self._scroll_speed_spin = QSpinBox()
        self._scroll_speed_spin.setRange(0, 500)
        self._scroll_speed_spin.setValue(self._cfg.get("scroll_speed", 60))
        self._scroll_speed_spin.setSuffix(" px/s")
        self._scroll_speed_spin.valueChanged.connect(self._on_change)
        anim_row.addWidget(self._scroll_speed_spin)
        ...
        anim_row.addWidget(QLabel(t("subwin_entry_anim")))
        ...
        anim_row.addWidget(self._entry_anim_combo)

        anim_row.addWidget(QLabel(t("subwin_exit_anim")))
        ...
        anim_row.addWidget(self._exit_anim_combo)

        anim_row.addWidget(QLabel(t("subwin_anim_duration")))
        ...
        anim_row.addWidget(self._anim_duration_spin)
    ...
    def _update_color_controls_state(self):
        ...
        self._opacity_slider.setEnabled(not has_image)
    ...
    def get_config(self) -> dict:
        cfg = {
            **self._cfg,
            ...
            "scroll_speed": self._scroll_speed_spin.value(),
...
class SubtitleSettingsDialog(QDialog):
    """Settings dialog for subtitle window."""
    ...
    def _build_ui(self):
        ...
        # === Basic ===
        basic_group = QGroupBox(t("subwin_basic"))
        bg_layout = QVBoxLayout(basic_group)

        row = QHBoxLayout()
        row.addWidget(QLabel(t("subwin_sentences")))
        self._sentences_spin = QSpinBox()
        self._sentences_spin.setRange(1, 10)
        self._sentences_spin.setValue(self._settings.get("sentences", 1))
        self._sentences_spin.valueChanged.connect(self._on_change)
        row.addWidget(self._sentences_spin)

        row.addWidget(QLabel(t("subwin_line_spacing")))
        ...
        row2 = QHBoxLayout()
        row2.addWidget(QLabel(t("subwin_window_width")))
        ...
        self._scroll_layout.addWidget(basic_group)

        # === Background ===
        bg_group = QGroupBox(t("subwin_background"))
        bg2_layout = QVBoxLayout(bg_group)
        ...
        bg_row.addWidget(self._bg_opacity_label_title)
        self._bg_opacity_slider = QSlider(Qt.Orientation.Horizontal)
        self._bg_opacity_slider.setRange(0, 255)
        self._bg_opacity_slider.setValue(self._settings.get("bg_opacity", 0))
        self._bg_opacity_slider.setFixedWidth(120)
        self._bg_opacity_slider.valueChanged.connect(self._on_bg_opacity_change)
        bg_row.addWidget(self._bg_opacity_slider)

        self._bg_opacity_label = QLabel(self._format_opacity(self._settings.get("bg_opacity", 0)))
        self._bg_opacity_label.setFixedWidth(36)
        bg_row.addWidget(self._bg_opacity_label)

        bg_row.addStretch()
        bg2_layout.addLayout(bg_row)

        # Window background image
        self._bg_color_controls = [
            self._bg_color_label, self._bg_color_btn,
            self._bg_opacity_label_title, self._bg_opacity_slider, self._bg_opacity_label,
        ]
        img_row, self._win_bg_image_edit = _make_image_row(
            self._settings.get("bg_image", ""), self._on_win_bg_image_change
        )
        bg2_layout.addLayout(img_row)

        self._update_win_bg_controls_state()

        self._scroll_layout.addWidget(bg_group)

        # === Animation ===
        anim_group = QGroupBox(t("subwin_animation"))
        anim_layout = QVBoxLayout(anim_group)

        anim_row = QHBoxLayout()
        anim_row.setSpacing(6)

        anim_row.addWidget(QLabel(t("subwin_auto_hide")))
        ...
        anim_row.addWidget(self._auto_hide_spin)

        anim_row.addWidget(QLabel(t("subwin_hide_animation")))
        ...
        for label, val in hide_anim_items:
        ...
        anim_row.addWidget(self._hide_anim_combo)

        anim_row.addWidget(QLabel(t("subwin_hide_duration")))
        ...
        anim_row.addWidget(self._hide_duration_spin)

        anim_row.addStretch()
        anim_layout.addLayout(anim_row)

        self._scroll_layout.addWidget(anim_group)

        # === Text Lines ===
        self._lines_group = QGroupBox(t("subwin_text_lines"))
        self._lines_layout = QVBoxLayout(self._lines_group)
        self._lines_layout.setSpacing(8)
    ...
    @staticmethod
    def _format_opacity(value):
        return f"{round(value / 255 * 100)}%"

    def _on_bg_opacity_change(self, value):
        self._bg_opacity_label.setText(self._format_opacity(value))
        self._on_change()

    def _rebuild_lines(self):
        # Clean up old widgets and separators
    ...
    def _emit_settings(self):
        s = {
            ...
            "bg_opacity": self._bg_opacity_slider.value(),
